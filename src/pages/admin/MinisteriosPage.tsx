...
interface MinForm { name: string; description: string; emoji: string; liderUid: string }
const EMPTY: MinForm = { name:'', description:'', emoji:'⛪', liderUid:'' }

function MinisterioModal({ min, onClose, onSave }: { min: Ministerio | null; onClose: () => void; onSave: (f: MinForm, id?: string) => Promise<void> }) {
  const [form, setForm]   = useState<MinForm>(min ? { name:min.name, description:min.description, emoji:min.emoji, liderUid:min.liderUid ?? '' } : EMPTY)
        ...
        <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
          <div>
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
              {EMOJIS.map(em => (
                <button key={em} type="button" onClick={() => set('emoji', em)} style={{ width:38, height:38, borderRadius:7, border:`2px solid ${form.emoji===em ? C.primary : C.line}`, background: form.emoji===em ? 'rgba(196,82,26,0.15)' : C.bg3, fontSize:18, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }}>{em}</button>
              ))}
            </div>
          </div>
          <div>
...
export default function MinisteriosPage() {
  ...
  async function handleSave(form: MinForm, id?: string) {
    const payload = { name:form.name.trim(), description:form.description.trim(), emoji:form.emoji, liderUid:form.liderUid || null, active:true }
    if (id) {
    ...
    <div style={{ fontFamily:'"Inter",system-ui,sans-serif' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28, flexWrap:'wrap', gap:16 }}>
        ...
        <button onClick={() => setEditing('new')} className="btn-primary" style={{ fontSize:14, padding:'10px 20px', display:'flex', alignItems:'center', gap:8 }}>
          <span style={{ fontSize:18 }}>+</span> Novo ministério
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding:'56px 24px', textAlign:'center' }}>
          ...
          <button onClick={() => setEditing('new')} className="btn-primary" style={{ fontSize:14, padding:'11px 24px' }}>+ Criar primeiro ministério</button>
        </div>
      ) : (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px,1fr))', gap:12 }}>
          {mins.map(m => (
            ...
            >
              <div style={{ height:80, background:`linear-gradient(160deg,${C.bg3},rgba(196,82,26,0.1))`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:44 }}>{m.emoji}</div>
              <div style={{ padding:'18px 20px' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <span style={{ fontSize:12, color:C.gray3 }}>👥 {m.membersCount ?? 0} membros</span>
                  <div style={{ display:'flex', gap:8 }}>
                    ...
                    >Editar</button>
                    <button onClick={() => handleDelete(m.id)} style={{ width:32, height:32, background:'none', border:`1px solid ${C.line}`, borderRadius:5, color:C.gray3, fontSize:15, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }}

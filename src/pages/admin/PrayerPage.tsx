...
import { Pill, Users as FamilyIcon, Wallet, Bird, HandHelping, Sparkle } from 'lucide-react'
import type { PrayerRequest, PrayerCategory } from '@/types'

const CATEGORIES: { key: PrayerCategory; label: string; Icon: typeof Pill; color: string }[] = [
  { key:'cura',       label:'Cura',        Icon:Pill,       color:'#52B788' },
  { key:'familia',    label:'Família',     Icon:FamilyIcon, color:'#E06A2C' },
  { key:'provisao',   label:'Provisão',    Icon:Wallet,     color:'#D4A84B' },
  { key:'libertacao', label:'Libertação',  Icon:Bird,       color:'#5B9EC9' },
  { key:'gratidao',   label:'Gratidão',    Icon:HandHelping, color:'#9b59b6' },
  { key:'outro',      label:'Outro',       Icon:Sparkle,    color:'#B09880' },
]
        ...
        <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
          <div>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {CATEGORIES.map(c => (
                <button key={c.key} type="button" onClick={() => setCat(c.key)} style={{ padding:'7px 14px', borderRadius:5, border:`1px solid ${cat===c.key ? c.color : C.line}`, background: cat===c.key ? `${c.color}18` : C.bg3, color: cat===c.key ? c.color : C.gray2, fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'"Inter",system-ui,sans-serif', transition:'all 0.15s', display:'inline-flex', alignItems:'center', gap:6 }}>
    ...
    <div style={{ fontFamily:'"Inter",system-ui,sans-serif' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28, flexWrap:'wrap', gap:16 }}>
        ...
        <button onClick={() => setShowNew(true)} className="btn-primary" style={{ fontSize:14, padding:'10px 20px', display:'flex', alignItems:'center', gap:8 }}>
          <Icons.plus size={16} /> Registrar pedido
        </button>
      </div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap', alignItems:'center' }}>
        ...
        {CATEGORIES.map(c => (
          <button key={c.key} onClick={() => setFilterCat(c.key)} style={{ padding:'7px 14px', borderRadius:5, border:`1px solid ${filterCat===c.key ? c.color : C.lineHi}`, background: filterCat===c.key ? `${c.color}18` : 'none', color: filterCat===c.key ? c.color : C.gray2, fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'"Inter",system-ui,sans-serif', minHeight:40, display:'inline-flex', alignItems:'center', gap:6 }}>
            <c.Icon size={14} strokeWidth={1.75} /> {c.label}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding:'48px 24px', textAlign:'center' }}>
          <div style={{ display:'flex', justifyContent:'center', marginBottom:10, color:C.gray3 }}><Icons.prayer size={36} /></div>
          ...
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: p.answered ? `rgba(82,183,136,0.05)` : C.bg2, border:`1px solid ${p.answered ? 'rgba(82,183,136,0.2)' : C.line}`, borderRadius:8, padding:'20px', display:'flex', gap:16, transition:'border-color 0.2s' }}>
                {/* Cat icon */}
                <div style={{ width:42, height:42, borderRadius:8, background:`${cat.color}18`, border:`1px solid ${cat.color}33`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, color:cat.color }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, flexWrap:'wrap' }}>
                    ...
                    {p.answered && <span style={{ background:'rgba(82,183,136,0.15)', color:'#52B788', fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:3, letterSpacing:'1px', textTransform:'uppercase', display:'inline-flex', alignItems:'center', gap:4 }}><Icons.checkCircle size={10} /> Respondido</span>}
                  </div>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                    ...
                    ><Icons.prayer size={14} /> Orar · {p.prayerCount}</button>
                    {!p.answered && (
                      <button onClick={() => markAnswered(p.id)} style={{ display:'flex', alignItems:'center', gap:6, background:'none', border:'1px solid rgba(82,183,136,0.3)', borderRadius:5, padding:'6px 14px', color:'#52B788', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'"Inter",system-ui,sans-serif' }}>
                        <Icons.checkCircle size={13} /> Marcar respondido
                      </button>
                    )}
                  </div>
                </div>
